...
const DeleteExperienceButton = ({
  ...
  const handleDelete = async () => {
    removeUserObject(id, 'experiences');
    if (imageUrl) {
      const img = imageUrl.split('/').pop();
      await supabase.storage.from('experiences').remove([img!]);
    }
  };
  return (
    <Dialog>
      ...
      <DialogContent className='dark:border-darkbg-2 dark:bg-darkbg-1 '>
        <DialogHeader>
          ...
          <DialogDescription className='mt-2 dark:text-natural-6'>
            You will not be able to recover this experience.
          </DialogDescription>
        </DialogHeader>
        <DialogFooter className='sm:justify-start'>
          <DialogClose asChild>
            <Button
              ...
              variant='destructive'
            >
              <FaRegTrashCan /> Delete this Experience
            </Button>
          </DialogClose>
        </DialogFooter>
      </DialogContent>
